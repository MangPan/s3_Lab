...
public class FileService {
    ...
    public FileRecordResponse completeUpload(String fileId){
        ...
        FileRecord fileRecord = fileRecordRepository.findById(fileId)
            .orElseThrow(() -> new IllegalArgumentException("file history not exists. fileId=" + fileId));
        ...
        try{
            ...
            throw new IllegalStateException("아직 S3에 파일이 업로드되지 않았습니다. key=" + fileRecord.getKey());
        }
    }
    ...
    public PresignedGetUrlResponse createPresignedGetUrlByFileId(String fileId){
        FileRecord fileRecord = fileRecordRepository.findById(fileId)
            .orElseThrow(() -> new IllegalArgumentException("파일 기록을 찾을 수 없습니다. fileId=" + fileId));
        
        if(fileRecord.getStatus() != FileStatus.UPLOADED){
            throw new IllegalStateException("업로드 완료된 파일만 다운로드 URL을 발급할 수 있습니다");
        }
